...
struct AssetComparisonEngine {
    ...
    func loadAssets(forecasts: [BennerCycleEntry]) async -> [ComparisonAssetSeries] {
        var series: [ComparisonAssetSeries] = []

        for asset in ComparisonAsset.allCases {
            let history = await makeHistory(for: asset)
    ...
    private func makeHistory(for asset: ComparisonAsset) async -> [ComparisonPoint] {
        guard let instrument = asset.marketInstrument else { return [] }
    ...
    private func makeProjection(for asset: ComparisonAsset, forecasts: [BennerCycleEntry], history: [ComparisonPoint]) -> [ComparisonPoint] {
        ...
        return forecasts.filter { $0.year >= last.year }.map { entry in
